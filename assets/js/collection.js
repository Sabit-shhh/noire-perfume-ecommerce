...
document.querySelectorAll('a,button,.frag-card,.spot-mini,.hstat').forEach(el => {
...
document.querySelectorAll('.f-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        const cards = [...document.querySelectorAll('.frag-card')];
        let count = 0;
        cards.forEach(c => {
            ...
            if (show) {
                ...
                setTimeout(() => c.classList.add('v'), count * 60);
                count++;
            }
        });
        document.getElementById('fcount').textContent = 'Showing ' + count + ' ' + (count === 1 ? 'fragrance' : 'fragrances');
    });
});